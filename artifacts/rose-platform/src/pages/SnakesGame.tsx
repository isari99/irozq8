...
function GameBoard({ players }: { players: Player[] }) {
  ...
  const drawLadder = (from: number, to: number) => {
    ...
    return (
      <g key={`ldr-${from}`}>
        <line x1={a.x+nx+0.022} y1={a.y+ny+0.022} x2={b.x+nx+0.022} y2={b.y+ny+0.022} stroke="rgba(0,0,0,0.25)" strokeWidth="0.13" strokeLinecap="round" />
        <line x1={a.x-nx+0.022} y1={a.y-ny+0.022} x2={b.x-nx+0.022} y2={b.y-ny+0.022} stroke="rgba(0,0,0,0.25)" strokeWidth="0.13" strokeLinecap="round" />
        <line x1={a.x+nx} y1={a.y+ny} x2={b.x+nx} y2={b.y+ny} stroke="#d4922a" strokeWidth="0.11" strokeLinecap="round" />
        <line x1={a.x-nx} y1={a.y-ny} x2={b.x-nx} y2={b.y-ny} stroke="#d4922a" strokeWidth="0.11" strokeLinecap="round" />
        <line x1={a.x+nx} y1={a.y+ny} x2={b.x+nx} y2={b.y+ny} stroke="rgba(255,255,255,0.35)" strokeWidth="0.032" strokeLinecap="round" />
        {rungs.map((t, i) => {
          const x1=(a.x+nx)+(b.x-a.x)*t, y1=(a.y+ny)+(b.y-a.y)*t;
          const x2=(a.x-nx)+(b.x-a.x)*t, y2=(a.y-ny)+(b.y-a.y)*t;
          return (
            <g key={i}>
              <line x1={x1+0.01} y1={y1+0.01} x2={x2+0.01} y2={y2+0.01} stroke="rgba(0,0,0,0.20)" strokeWidth="0.095" strokeLinecap="round" />
              <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#c08830" strokeWidth="0.09" strokeLinecap="round" />
              <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="rgba(255,255,255,0.26)" strokeWidth="0.030" strokeLinecap="round" />
            </g>
          );
        })}
          ...
          return (
            <div key={num} className="relative overflow-hidden select-none"
              ...
              <span className="absolute font-black leading-none"
                style={{ top:"2px", left:"3px", fontSize:"clamp(5px,1.2cqw,11px)",
                  color:"#222", zIndex:10 }}>
                {toArabic(num)}
              </span>
              {isGoal && (
                <div className="absolute inset-0 flex items-center justify-center"
                  ...
                  <motion.span style={{ fontSize:"clamp(10px,2.4cqw,26px)", filter:"drop-shadow(0 0 8px gold)" }}
                  ...
                  {playersHere.map(p => {
                    ...
                    return (
                      <motion.div key={p.username}
                        ...
                        style={{ width:s, paddingBottom:s, position:"relative",
                          borderRadius:"50%", overflow:"hidden",
                          border:`2.5px solid ${p.color}`,
                          boxShadow:`0 0 10px ${p.color}cc`, flexShrink:0 }}>
...
export default function SnakesGame() {
  ...
  const endTurn = useCallback((pidx: number) => {
    ...
    setCurrentPlayerIdx(nextIdx); currentIdxRef.current = nextIdx;
    isAnimRef.current = false; setIsAnimating(false); setLastAction(null);
  }, []);

  const triggerWin = useCallback((pidx: number) => {
    const w = playersRef.current[pidx];
    setWinner(w); setPhase("finished"); phaseRef.current = "finished";
  ...
  const handleSpecial = useCallback((pidx: number, pos: number) => {
    ...
    if (LADDERS[pos] !== undefined) {
      ...
      setTimeout(() => {
        setPlayers(prev => { const n=prev.map((p,i)=>i===pidx?{...p,position:dest}:p); playersRef.current=n; return n; });
  ...
  const startMove = useCallback((pidx: number, steps: number) => {
    ...
    const tick = () => {
      ...
      step++; pos = Math.min(pos + 1, 100);
      setPlayers(prev => { const n=prev.map((p,i)=>i===pidx?{...p,position:pos}:p); playersRef.current=n; return n; });
  ...
  const doRoll = useCallback(() => {
    ...
    setIsRolling(true); setIsAnimating(true); isAnimRef.current = true; setLastAction(null);
    let count = 0;
    const iv = setInterval(() => {
      setDiceValue(Math.floor(Math.random() * 6) + 1);
      count++;
      if (count >= 10) {
        clearInterval(iv); setDiceValue(roll); setIsRolling(false);
  ...
  // ─────────────────────────────────────────────────────────────────────────
  return (
    // ← Same base class as XOGame: gradient-bg + overflow-hidden
    <div className="h-screen gradient-bg flex flex-col overflow-hidden" dir="rtl"
      style={{ fontFamily:"'Cairo',sans-serif" }}>

      {/* ── HEADER — identical style to XOGame ─────────────────────────── */}
      <header
      ...
      <div className="flex-1 overflow-hidden flex z-10">
        <AnimatePresence mode="wait">
          ...
          {/* ══ JOINING — same layout as XOGame joining screen ══ */}
          {phase === "joining" && (
            <motion.div key="joining"
              ...
              <div className="text-center space-y-3">
                {/* Connection status — same badge as XO */}
              ...
              {/* Player cards — same style as XO joining slots */}
              {players.length > 0 && (
          ...
          {phase === "playing" && (
            <motion.div key="playing"
              ...
              {/* ── SIDEBAR — same dark glass as XO cards ── */}
              <div style={{ width:"200px", flexShrink:0 }}
                ...
                {/* Dice — same dark as XO board cells */}
                <div className="rounded-b-2xl p-3 flex-shrink-0 flex flex-col items-center gap-2 border"
                  ...
                  {!isAnimating && currentPlayer ? (
                    ...
                    <p className="text-[10px] font-bold text-purple-300/50">جارٍ الحركة...</p>
                  )}
                </div>
              ...
              {/* ── BOARD — no extra wrapper background ── */}
      ...
      <AnimatePresence>
        {phase === "finished" && winner && (
          <motion.div key="win"
            ...
            {/* Winner avatar with rings — same as XO result screen */}
